...
$sa1=mysql_query("UPDATE `json` SET `status`='".$role2."' WHERE `id`='".$r[0]."'");
...
$start=0;
	...
	//$sql=mysql_query("SELECT `firstname`, `lastname`, `email`,`role1` FROM `register1` where role1<>'superadmin'",$conn);
	$i=0;
	$firstName=array();
	$lastname=array();
	$email=array();
	$role=array();
	$sql=mysql_query("select id,visname,status from json LIMIT $start, $limit ",$conn);
	$num=mysql_num_rows($sql);
	while($row=mysql_fetch_array($sql))
	{
		$id[$i]=$row['id'];
		$visname[$i]=$row['visname'];
		$status[$i]=$row['status'];
...
echo'<tr style="background-color: #367fa9;font-size: 19px;font-family: cursive;color-rendering: optimizeQuality;color: #fff;">';
	...
	$rows=mysql_num_rows(mysql_query("select id,visname,status from json"));
//pagination logic
